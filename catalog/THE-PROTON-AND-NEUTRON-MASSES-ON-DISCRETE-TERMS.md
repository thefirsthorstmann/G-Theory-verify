...
**The root.** The bridge maps the measured nucleon average to the tail axis exactly:

```
0.036 × ( m̄ / 1.008 )  =  m̄ / 28  =  0.0359989533(1)
```

where m̄ is the measured nucleon average in u. This is an identity, not a fit. The value 137.0359989533(1) is therefore the unison row of any transport between the mass and fine-structure residuals, and it is registered here under the name the mechanism gives it: the fine-structure value with no transport applied. Every current determination sits above it.
