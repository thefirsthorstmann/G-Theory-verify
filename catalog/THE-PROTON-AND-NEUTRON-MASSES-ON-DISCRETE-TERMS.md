...
## 10 · The accumulation mechanism: one syntonic comma per octave
...
**The quantitative state of the hypothesis, plainly.** With the count of Section 9, the per-octave increment required by the neutron's displacement is 0.0125166(10), which exceeds the syntonic excess 1/80 by 0.133(8) percent. Equivalently: an exactly syntonic increment requires a seating temperature of 162.1 MeV, which the two crossover determinations exclude at 3.8 and 6.9 standard deviations. Against the wider lattice bar the discrepancy factor is 1.0360(99), and seventeen superparticular steps lie within one standard deviation of it. The two adjacent to the bridge number, 28/27 and 29/28, are noted as the construction's own — a stated selection, not an inference.

**The seating increment.** The excess admits a derived candidate, stated here with its exposure. Write the accumulated load as (N + X)/80 — one syntonic comma per octave, plus a one-time increment X at seating. The measured excess is X = 0.0521(32) octaves. The pre-registered class of one-time steps is the logarithms base two of the 7-smooth superparticular ratios and the standard commas. Exactly two candidates lie within three standard deviations: log₂(28/27) at 0.1 σ, and log₂(25/24) at 2.1 σ, nineteen times farther. Each candidate carries structure the construction recognizes, and the paper registers both. The septimal case: 28/27 = (7/6)/(9/8), the step between a septimal interval and its three-limit neighbour, is the unique licensed member of the tighter band. And 9/7 × 28/27 = 4/3 exactly — the septimal third carried exactly onto the subdominant, the destination of the transferred fraction in the partition above: the step at seating is the seed's own. The five-limit case: 25/24 = (5/4)/(6/5), the chromatic semitone, is the smaller of the two in its parts and the superparticular at the frame's root 24. It lies in the same prime limit as the comma added each octave thereafter. The present table prefers the septimal step nineteen-fold in distance; the smaller parts and the shared limit favour the chromatic; the measurement, not the preference, will decide. The reading is one sentence either way: seating is increment zero. Commitment is the thermodynamically costly operation in the series' account of matter [22], and each seated object receives one step at its own seating, partitioned 23 to 9 like every increment after it. A increment landing on the neutron alone fails the measured ratio at sixteen standard deviations; the partitioned increment leaves 23/32 exact, as measured. Under either step the two faces of the discrepancy become one identity. The load counts from one step above m_p/6: 14m_p/81 = 162.2 MeV for the septimal step, 25m_p/144 = 162.9 for the chromatic. The crossover the lattice observes remains m_p/6, and the tension between the exact syntonic and the lattice dissolves, because the initial increment does not enter the crossover. The candidates forbid differently. As the neutron mass sharpens, the septimal step requires Δn to approach 0.4916103, a tenth of a standard deviation from today's value; the chromatic step requires 0.4916906, two standard deviations from it. The next adjustment separates them. A consolidated value away from both refutes the increment-zero reading, and the fallback resolutions remain as before: the lattice moves, or the excess is real and unexplained. The ratio 23/32 is unaffected by any of this, because a uniform excess — and a partitioned initial increment — cancel in the ratio.

## 11 · The fine-structure side: a flux, not an accumulation

Section 6 established two decompositions: the mass seats are sums, 100800 − 72 and 100800 + 66, whose borrows propagate across digit positions; the fine-structure seat is a concatenation, the six-digit string split at its midpoint into 137 | 036. A sum stores a stationary address; a midpoint split of a period-six string is the form of a reciprocal read mid-cycle, and it flows. The published reading of the run of nines as a borrow in flight [9] identifies the fine-structure residual as a flux maintained by transfer rather than an accumulated quantity, and this section treats it so.
...
The count of Section 9 locates the present inside the mechanism. The elapsed 39.2764 octaves place the present 27.6 percent of the way through the fortieth octave since seating. The thirty-ninth increment completed at redshift 0.21, about 2.6 billion years ago; the fortieth arrives when the universe has expanded by a further factor of 1.65, roughly seven billion years from now. The increments are uniform in octaves and strongly non-uniform in elapsed time: the first landed about twenty microseconds after the origin, and the current one has been underway for billions of years. Stated in years for orientation, with the standard radiation-era scaling: roughly ten increments completed within the first ten seconds, fifteen within the first two hours, and twenty within the first two months. Twenty-nine had completed by the release of the microwave background, and the ten since occupy the thirteen point eight billion years of the matter and dark-energy eras — the fortieth alone spanning roughly nine and a half billion. In the octave coordinate the development is uniform; in elapsed time it is compressed at the origin and dilated without bound toward the floor. In the series' account this asymmetry is the reading of time itself: time is succession — the count of operations — rather than a container [22]. The octave count is the mechanism's native clock, and the year is a derived coordinate, borrowed from the present epoch's orbit.

The two coordinates carry very different precision. In years the age is a two-figure number: the same count converts to 13.8, 13.3, or 12.9 billion years under the Planck, registered, and distance-ladder packages, and the spread belongs entirely to the borrowed ruler. In octaves the age since seating is 39.2764(3): six significant figures, eight parts per million, limited only by the background temperature's half-millikelvin. Each ten-fold improvement in that temperature adds one digit, and a microkelvin-class measurement delivers nine figures. The age of the universe is a precision quantity in the coordinate the mechanism runs on, and a two-figure estimate in the coordinate it borrows. The companion monograph's seventh volume locates the present at the crest of the energy budget by an independent construction; the octave coordinate above is the mechanism's own and is stated beside it, not derived from it.
...
## 13 · Excluded variants, and the stated selections

Each of the following was tested against the same data and fails at the stated size. They are listed so that they are not rediscovered. The selections the paper itself makes are also enumerated here, in one place. The arrangement of the seat pair is one in four; the seating divisor, thirteen percent at its declaration threshold; the superparticular naming at the increment, two of seventeen. The seating-increment step is one of two candidates inside three standard deviations, the nearer by a factor of nineteen. The ratio's denominator bound is fixed by the rung grid rather than chosen. The registered values of Section 14 carry the exposure forward either way.
...
2. The exactly syntonic increment. Requires a seating temperature of 162.1 MeV; excluded by the two crossover determinations at 3.8 σ and 6.9 σ.
...
4. The share model, in which a fraction of the transferred flow lands on the axis. It reproduces the neutron's increment excess as 1/768 to +0.3 σ, and then fails its own second consequence, the proton's displacement, at 565 σ.
5. The six-increment steady state for the fine-structure flux: excluded at ≥3.4 σ by every determination.
6. Exact integer increment counts (28 and 39): excluded at 11 σ.
...
2. The elapsed count: 39.2764 octaves from the exact form, a pure number moving only with the two anchors; 39.28(1) if read from the measured crossover instead. The count is known today to eight parts per million, one added digit per ten-fold improvement of the background temperature.
...
7. The living clock, in two clauses. Between increments the masses are constant: a present-day proton-to-electron drift confirmed at current laboratory sensitivity refutes the event-coincident law. Across increments the ratio steps: it stood 0.9 × 10⁻⁷ higher at redshift 0.886 than today, a sign-definite prediction at the current bar of that sightline. The smaller early splitting raises primordial helium at the several-tenths-of-a-percent level, an open account with its direction fixed.
...
9. The seating increment: increment zero is one licensed step — log₂(28/27) or log₂(25/24) octaves of syntonic excess. As the neutron mass sharpens, the septimal step requires Δn → 0.4916103 and the chromatic step 0.4916906; the next adjustment separates them, a value away from both refutes the increment-zero reading, and the partition 23/32 must hold through it.

## 15 · Verification: the public batteries

Every exact identity, every scan, every exclusion, and every registered value in this paper is pinned by mechanical batteries: fifty-two tests in three files, written alongside the work. The full program suite of 2,776 tests was re-run green before this manuscript was composed. The batteries are public, together with this paper's source, at github.com/thefirsthorstmann/g-theory-verify, where the reader can re-derive every number in this paper from the CODATA table and the two published anchors.
