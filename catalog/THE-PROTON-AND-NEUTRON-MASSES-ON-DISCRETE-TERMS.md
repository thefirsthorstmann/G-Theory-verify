...
**The gate toll.** The excess admits a derived candidate, stated here with its exposure. Write the accumulated load as (N + X)/80 — one syntonic comma per octave, plus a one-time deposit X at seating. The measured excess is X = 0.0521(32) octaves. The pre-registered class of one-time steps is the logarithms base two of the 7-smooth superparticular ratios and the standard commas. Exactly two candidates lie within three standard deviations: log₂(28/27) at 0.1 σ, and log₂(25/24) at 2.1 σ, nineteen times farther. Each candidate carries structure the construction recognizes, and the paper registers both. The septimal case: 28/27 = (7/6)/(9/8), the step between a septimal interval and its three-limit neighbour, is the unique licensed member of the tighter band. And 9/7 × 28/27 = 4/3 exactly — the septimal third carried onto the subdominant, the mechanism's own door: the seed pays the seed's step at the gate. The five-limit case: 25/24 = (5/4)/(6/5), the chromatic semitone, is the smaller of the two in its parts and the superparticular at the frame's root 24. It lives in the same limit as the comma deposited ever after — the toll in the currency of the rent. The present table prefers the septimal step nineteen-fold in distance; the smaller parts and the shared limit speak for the chromatic; the measurement, not the preference, will decide. The reading is one sentence either way: seating is receipt zero. Commitment is the costly operation in the series' account of matter [22], and the object pays one step at the gate of the six-frame, partitioned 23 to 9 like every receipt after it. A toll landing on the neutron alone fails the measured ratio at sixteen standard deviations; the partitioned toll leaves 23/32 exact, as measured. Under either toll the two faces of the discrepancy become one identity. The load counts from one step above m_p/6: 14m_p/81 = 162.2 MeV for the septimal step, 25m_p/144 = 162.9 for the chromatic. The crossover the lattice observes remains m_p/6, and the tension between the exact syntonic and the lattice dissolves, because the lattice never sees the gate. The candidates forbid differently. As the neutron mass sharpens, the septimal step requires Δn to approach 0.4916103, a tenth of a standard deviation from today's value; the chromatic step requires 0.4916906, two standard deviations from it. The next adjustment separates them. A consolidated value away from both refutes the gate reading, and the fallback resolutions remain as before: the lattice moves, or the excess is real and unexplained. The ratio 23/32 is unaffected by any of this, because a uniform excess — and a partitioned toll — cancel in the ratio.
...
9. The gate toll: the one-time seating deposit is one licensed step at the gate — log₂(28/27) or log₂(25/24) octaves of syntonic excess, receipt zero. As the neutron mass sharpens, the septimal step requires Δn → 0.4916103 and the chromatic step 0.4916906; the next adjustment separates them, a value away from both refutes the gate reading, and the partition 23/32 must hold through it.
